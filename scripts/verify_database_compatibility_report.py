#!/usr/bin/env python3
"""Fail CI when cross-database integration tests are missing or skipped."""
...
DEFAULT_REPORT = (
    ...
    / "target"
    / "surefire-reports"
    / "TEST-io.clusterinfra.rca.webconsole.DatabaseCompatibilityTests.xml"
)
EXPECTED_TESTS = {
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Verify PostgreSQL and MariaDB compatibility tests actually ran."
    )
...
def evaluate_report(path: Path) -> dict[str, object]:
    ...
    if not path.is_file():
        result["errors"] = ["Surefire report was not found."]
        return result

    try:
        root = ET.parse(path).getroot()
    except (ET.ParseError, OSError) as error:
        result["errors"] = [f"Surefire report could not be parsed: {error}"]
        return result

    testcases = {case.get("name", ""): case for case in root.findall("testcase")}
    missing = sorted(EXPECTED_TESTS.difference(testcases))
    skipped = sorted(
        name for name, case in testcases.items()
    ...
    if skipped:
        errors.append(f"Database compatibility tests were skipped: {', '.join(skipped)}")
    if failed:
        errors.append(f"Database compatibility tests failed: {', '.join(failed)}")
