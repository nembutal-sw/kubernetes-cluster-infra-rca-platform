...
public class GiteaWebhookService {
    ...
    public GitOpsWebhookResult handle(
        ...
        if (!changes.claimWebhookDelivery(deliveryId, PROVIDER, event)) {
            ...
            throw new ResponseStatusException(CONFLICT, "Gitea webhook delivery was already processed");
        }
        ...
        if (repository.isBlank() || pullRequestNumber <= 0 || pullRequest.isMissingNode()) {
            throw new ResponseStatusException(BAD_REQUEST, "Gitea pull_request payload is incomplete");
        }
    ...
    private void validateHeaders(String event, String deliveryId) {
        if (event == null || event.isBlank() || event.length() > 64) {
            throw new ResponseStatusException(BAD_REQUEST, "X-Gitea-Event is required");
        }
        if (deliveryId == null || deliveryId.isBlank() || deliveryId.length() > 128) {
            throw new ResponseStatusException(BAD_REQUEST, "X-Gitea-Delivery is required");
        }
    }
    ...
    private void verifySignature(String suppliedSignature, byte[] body) {
        ...
        if (!config.isEnabled() || !PROVIDER.equalsIgnoreCase(config.getProvider()) || secret.isBlank()) {
            throw new ResponseStatusException(SERVICE_UNAVAILABLE, "Gitea webhook verification is not configured");
        }
        if (suppliedSignature == null || suppliedSignature.isBlank()) {
            throw new ResponseStatusException(UNAUTHORIZED, "invalid Gitea webhook signature");
        }
        try {
            ...
            throw new IllegalStateException("could not verify Gitea webhook signature", exception);
        }
    }
    ...
    private JsonNode parse(byte[] body) {
        try {
            ...
            throw new ResponseStatusException(BAD_REQUEST, "Gitea webhook body is invalid JSON");
        }
    }
