...
`preprocessed_evidence`, `derived_signals`, `resolution_checklist`는 report의 `evidence` 배열 안에 별도 section으로 들어갑니다. 저장 스키마는 그대로 두고, report 소비자가 원본 collector 결과, LLM 입력 payload, 분석 결과를 같은 응답에서 볼 수 있게 하기 위한 구조입니다.
