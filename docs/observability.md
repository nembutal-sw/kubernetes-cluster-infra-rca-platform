...
LLM p95 임계값은 단일 성공 실행으로 조정하지 않습니다. `scripts/llm-burn-in-campaign.py`로 호출 예산을 제한하고 `scripts/llm-burn-in-report.py`로 최소 20개 표본, 5개 장애 시나리오, 8시간 구간 3개를 확보한 뒤 전체 및 시나리오별 latency, 오류율, usage metadata와 LLM action 안전성을 함께 검토합니다. 기준을 채우지 못하면 60초 초기값을 유지합니다.
