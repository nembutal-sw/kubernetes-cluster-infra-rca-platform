...
```bash
pg_dump --format=custom --file=rca.dump "$RCA_JDBC_DATABASE"
...
관리자는 다음 API 또는 Web Console의 Rotate token 기능을 사용합니다.

```text
POST /api/clusters/{cluster_id}/agent-token/rotate
```

새 token은 한 번만 표시됩니다. 대상 클러스터의 Kubernetes Secret을 갱신한 뒤
DaemonSet을 순차 재시작합니다.
